...
@login_required
def deposit():
    btc= BTCWallet.query.filter_by(user_id=current_user.id).first()
    eth= ETHWallet.query.filter_by(user_id=current_user.id).first()
    usdt= USDTWallet.query.filter_by(user_id=current_user.id).first()
    bnb= BNBWallet.query.filter_by(user_id=current_user.id).first()
  
    session.pop('_flashes', None)
    return render_template('user/deposit.html', addresses=[
        {'coin': 'BTC', 'address': btc.deposit if btc else None},
        {'coin': 'ETH', 'address': eth.deposit if eth else None},
        {'coin': 'USDT', 'address': usdt.deposit if usdt else None},
        {'coin': 'BNB', 'address': bnb.deposit if bnb else None}
    ])
