...
def trade():
    ...
    if request.method == "POST":
        try:
            ...
            # Calculate profit (12% of traded amount)
            profit = traded_amount * Decimal("0.12")
            print(f"Calculated profit: {profit}")

            coin_enum = Coins(traded_coin)
            
            # Debugging: Check if current_user.account is loaded properly
            print(f"Current user account: {current_user.account}")
            ...
            # Check if the correct initial balance is set in the account table
            if balance == 0:
                print(f"Warning: Balance for {traded_coin} is 0, check the database to ensure it was initialized correctly.")
            ...
            setattr(current_user.account, coin_enum.name, new_balance)

            # Ensure that the account balance is committed to the database
            db.session.add(current_user.account)  # Explicitly add the account to session
            db.session.commit()  # Commit the updated balance

            # Check if the new balance was actually updated
            updated_balance = getattr(current_user.account, coin_enum.name)
            ...
            return jsonify({
                ...
                "profit": 0,
                "new_balance": float(getattr(current_user.account, traded_coin, 0)) if 'traded_coin' in locals() else 0
            })
